...
def play_hit_sound_thread_target(score):
    ...
    pygame.mixer.Channel(0).play(
        pygame.mixer.Sound(f'sound\hit_real{math.floor(SOUND_NUM_HIT_REAL * random.random())}.wav'))
    time.sleep(0.15)
    if score > 0 and score < 6:
        pygame.mixer.Channel(1).play(
            pygame.mixer.Sound(f'sound/hit_feedback{math.floor(SOUND_NUM_HIT_FEEDBACK * random.random())}.wav'))
    ...
    pygame.mixer.Channel(2).play(
        pygame.mixer.Sound(f'sound\shell_fall{math.floor(SOUND_NUM_SHELL_FALL * random.random())}.wav'))
    return
...
def main():
    ...
    set_windows()
    net_cam = 'http://192.168.137.203:4747/videos'
    usb_cam = 0
